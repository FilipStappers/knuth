...
@y
float alpha=1.001; /* magic constant for heuristic scores */
float gamm=0.21; /* magic ratio for the clause reduction heuristic */
...
@y
case 'g': k|=(sscanf(argv[j]+1,"%f",&gamm)-1);@+break;
case 't': k|=(sscanf(argv[j]+1,"%d",&theta64)-1);@+break;
@z
@x
...
if (k || hbits<0 || hbits>30 || buf_size<=0 || memk_max<2 || memk_max>31 ||
  ...
  fprintf(stderr,
     "Usage: %s [v<n>] [c<n>] [h<n>] [b<n>] [s<n>] [d<n>] [m<n>] ",argv[0]);
  fprintf(stderr," [g<f>] [a<f>] [t<n>] [p<n>] [q<n] [z<n>]");
  fprintf(stderr," [i<n>] [r<f>] [x<foo>] < foo.dat\n");
@z
@x
...
number clauses that contain~|l| and are not yet satisfied.
...
for (c=bclauses;c;c--) {
  ...
  if (j>=k-2) {
    if (cinx[c].size!=2)
      fprintf(stderr,"ex-big clause %d has size %d!\n",c,cinx[c].size);
    goto inactive;
  }
  if (cinx[c].size!=k-j)
    fprintf(stderr,"big clause %d has size %d not %d\n",c,cinx[c].size,k-j);
  continue;
...
for (l=2;l<badlit;l++) if (isfree(l)) {
  for (la=kinx[l].addr, ls=kinx[l].size;ls;la++,ls--) {
    ...
    if ((int)cinx[c].size<0)
      fprintf(stderr,"kinx[%s%.8s] includes active clause %d!\n",
                     varname(l),c);
  }
  for (;la<kinx[l-1].addr;la++) {
    c=kmem[la];
    if ((int)cinx[c].size>=0)
      fprintf(stderr,"kinx[%s%.8s] omits active clause %d!\n",
                     varname(l),c);
  }
}
...
for (l=2;l<badlit;l++) o,kinx[l].size=0; /* clear the counts */
...
if (out_file) {
  for (jj=0;jj<j;jj++) fprintf(out_file," %s%.8s",varname(cmem[la+jj]));
  fprintf(out_file,"\n");
}
if (j<3) { /* not big */
  k=la,u=cmem[la];
...
first time this happens to~|c| is, however, special: Then we also want to make
...
for (o,tla=kinx[tll].addr,tls=kinx[tll].size;tls;tla++,tls--) {
  ...
  if (cis==3) { /* exactly two literals of |c| are now free */
    ...
    if (verbose&show_details)
      fprintf(stderr,"  %s%.8s->%s%.8s|%s%.8s\n",
            varname(bar(tll)),varname(u),varname(v));
...
@y
for (o,la=kinx[bar(l)].addr,ls=kinx[bar(l)].size;ls;la++,ls--) {
  o,c=kmem[la];
  for (oo,ci=cinx[c].addr,cia=cinx[c-1].addr;ci<cia;ci++) {
    o,u=cmem[ci];
    if (o,stamp[thevar(u)]==real_truth+(u&1)) break;
  }
  if (ci==cia) goto nogood;
}
@z
@x
...
@ The remaining literal may have become fixed, but not yet removed
...
@<Put the remaining literal of |c| into |bstack|@>=
{
  ...
  if (la==ua) {
    bptr=-1;
    if (verbose&show_gory_details)
      fprintf(stderr,"  looking %s%.8s-> [%d]\n",
                            varname(ll),c);
  }@+else if (u) {
    o,bstack[bptr++].u=u;
    if (verbose&show_gory_details)
        fprintf(stderr,"  looking %s%.8s->%s%.8s [%d]\n",
                              varname(ll),varname(u),c);
  }
}
@z
@x
...
@ @<Put the remaining doublelook literal of |c| into |bstack|@>=
{
  ...
  if (la==ua) {
    bptr=-1;
    if (verbose&show_doubly_gory_details)
      fprintf(stderr,"  dlooking %s%.8s-> [%d]\n",
                            varname(ll),c);
  }@+else if (u) {
    o,bstack[bptr++].u=u;
    if (verbose&show_doubly_gory_details)
        fprintf(stderr,"  dlooking %s%.8s->%s%.8s [%d]\n",
                              varname(ll),varname(u),c);
  }
}
@z
...
for (c=1;c<=bclauses;c++) {
  fprintf(stderr,"%d:",c); /* show the reference number to the user */
  for (la=cinx[c].addr;la<cinx[c-1].addr;la++)
   fprintf(stderr," %s%.8s",varname(cmem[la]));
  fprintf(stderr,"\n");
}
...
clause_weight[2]=1.0;
for (k=3;k<max_clause;k++) o,clause_weight[k]=clause_weight[k-1]*gamm;
