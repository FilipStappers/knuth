...
float alpha=3.3; /* magic constant for heuristic scores */
float max_score=25.0; /* heuristic scores will be at most this */
...
int max_prelook_arcs=10000; /* space available for arcs re strong components */
int dl_max_iter=8; /* maximum iterations of double-look */
float dl_rho=0.9985; /* damping factor for the double-look trigger */
...
case 'm': k|=(sscanf(argv[j]+1,"%d",&memk_max)-1);@+break;
case 'a': k|=(sscanf(argv[j]+1,"%f",&alpha)-1);@+break;
case 't': k|=(sscanf(argv[j]+1,"%f",&max_score)-1);@+break;
case 'l': k|=(sscanf(argv[j]+1,"%d",&hlevel_max)-1);@+break;
case 'p': k|=(sscanf(argv[j]+1,"%d",&levelcand)-1);@+break;
case 'q': k|=(sscanf(argv[j]+1,"%d",&mincutoff)-1);@+break;
case 'z': k|=(sscanf(argv[j]+1,"%d",&max_prelook_arcs)-1);@+break;
case 'i': k|=(sscanf(argv[j]+1,"%d",&dl_max_iter)-1);@+break;
case 'r': k|=(sscanf(argv[j]+1,"%f",&dl_rho)-1);@+break;
...
if (k || hbits<0 || hbits>30 || buf_size<=0 || memk_max<2 || memk_max>31 ||
  ...
  fprintf(stderr,
     "Usage: %s [v<n>] [c<n>] [h<n>] [b<n>] [s<n>] [d<n>] [m<n>]",argv[0]);
  fprintf(stderr," [a<f>] [t<f>] [l<n>] [p<n>] [q<n] [z<n>]");
  fprintf(stderr," [i<n>] [r<f>] [x<foo>] < foo.dat\n");
  exit(-1);
}
...
void sanity(void) {
  ...
  for (k=0;k<vars;k++) {
    if (freevar[freeloc[k+1]]!=k+1)
      fprintf(stderr,"freeloc[%d] is wrong!\n",k+1);
    if (freeloc[freevar[k]]!=k)
      fprintf(stderr,"freevar[%d] is wrong!\n",k);
  }
  for (k=0;k<rptr;k++) {
    l=rstack[k];
    if (freeloc[thevar(l)]<freevars)
      fprintf(stderr,"literal %d on rstack is free!\n",l);
  }
  if (rptr+freevars!=vars)
    fprintf(stderr,"rptr=%d, freevars=%d, vars=%lld\n",rptr,freevars,vars);
...
for (l=2;l<badlit;l++) {
  ...
  for (k=0;k<los;k++) {
    if (tmem[tmem[tmem[la+k].link].link].link!=la+k)
      fprintf(stderr,"links clobbered in tmem[0x%x]!\n",
               la+k);     
    ...
    if (k<ls) { /* active area, shouldn't contain assigned variables */
      if (stamp[thevar(l)]<real_truth) { /* unless |l| itself is assigned */
        if (stamp[thevar(u)]>=real_truth)
          fprintf(stderr,"active timp u for free lit %d has assigned lit %d!\n",
                             l,u);
        if (stamp[thevar(v)]>=real_truth)
          fprintf(stderr,"active timp v for free lit %d has assigned lit %d!\n",
                             l,v);
      }
    }@+else if (stamp[thevar(u)]<real_truth && stamp[thevar(v)]<real_truth)
      fprintf(stderr,"inactive timp entry for %d has unassigned %d and %d!\n",
                           l,u,v);
  }
}
...
for (l=2;l<badlit;l++) o,timp[l].size=0; /* clear the counts */
...
for (l=2;l<badlit;l++) {
  ...
  if (la&((1<<k)-1))
    fprintf(stderr,"addr of bimp[%d] is clobbered (0x%x, k=%d)!\n",
                l,la,k);
  else if (bimp[l].alloc!=1<<k)
    fprintf(stderr,"alloc of bimp[%d] is clobbered (%d, k=%d)!\n",
                l,bimp[l].alloc,k);
  else if (bimp[l].size>bimp[l].alloc)
    fprintf(stderr,"size of bimp[%d] is clobbered (%d>%d)!\n",
                l,bimp[l].size,bimp[l].alloc);
  else if (la>=1<<memk)
    fprintf(stderr,"addr of bimp[%d] is out of bounds (0x%d>0x%d)!\n",
                l,la,1<<memk);
  else if (memfree(la))
    fprintf(stderr,"block 0x%x of bimp[%d] isn't reserved!\n",
                la,l);
  else for (j=bimp[l].size-1;j>=0;j--) if (mem[la+j]<2 || mem[la+j]>=badlit)
    fprintf(stderr,"literal %d in bimp[%d] is out of bounds!\n",
                mem[la+j],l);
}
for (k=2;k<memk;k++) {
  for (p=~mem[avail(k)];;p=~linkf(p)) {
    if ((p&((1<<k)-1)) && p!=avail(k))
      fprintf(stderr,"link in avail(%d) is clobbered (0x%x)!\n",
                k,p);
    else if (p>=1<<memk)
      fprintf(stderr,"link in avail(%d) is out of bounds (0x%d>0x%d)!\n",
                k,p,1<<memk);
    else if (kval(p)!=k)
      fprintf(stderr,"kval of 0x%x in avail(%d) is %d!\n",
                p,k,kval(p));
    else if (memfree(p^(1<<k)) && kval(p^(1<<k))==k)
      fprintf(stderr,"buddy of 0x%x in avail(%d) is also in that list!\n",
                p,k);
    else if (~linkf(~linkb(p))!=p)
      fprintf(stderr,"linking anomaly at 0x%x in avail(%d)!\n",
                p,k);
...
if (memk==memk_max) { /* oops, we're outta room */
  fprintf(stderr,"Sorry... more memory is needed! (Try option m%d.)\n",
              memk_max+1);
  exit(-666);
}
p=1<<memk;
...
if (memk>memk_max) { /* |memk_max| is too small even for empty lists! */
  fprintf(stderr,"The value of memk_max is way too small for %d literals!\n",
                lits);
  exit(-667);
}
...
if (bestlit) {
  ...
  fprintf(stderr,"Level %d: no branch\n",
                level);
...
case 2: fprintf(stderr,"real_truths:");@+break;
  ...
  }@+else fprintf(stderr,"truths at least %d:",
                         cs);
...
special_start:@+if (verbose&show_details)
    fprintf(stderr,"(lookahead for level %d forces %d)\n",
                             level,forcedlits);
  cs=near_truth;
  fptr=eptr=rptr;
  @<Bump |istamp| to a unique value@>;
...
For technical reasons related to the |work| array, a normal |bstamp|
should always exceed 32.
...
void hscores(float *h,float *hp) {
  ...
  for (j=0;j<freevars;j++) {
    ...
    if (verbose&show_scores)
      fprintf(stderr,"(%.8s: pos %.2f neg %.2f r=%.4g)\n",
...
for (j=0;j<freevars;j++) {
  ...
  if (isfree(x)) {
    ...
    l=poslit(x)+(stamp[x]&1);
common_case: @<If |l| implies any unsatisfied clauses, |goto nogood|@>;
  }
}
goto satisfied;
nogood:@;
...
for (o,la=timp[l].addr,ls=timp[l].size;ls;la++,ls--) {
  o,u=tmem[la].u, v=tmem[la].v;
  if ((o,stamp[thevar(u)]!=real_truth+(u&1)) && 
      (o,stamp[thevar(v)]!=real_truth+(v&1))) goto nogood;
}
...
for (oo,la=bimp[l].addr,ls=bimp[l].size,p=lmem[bar(l)].arcs;ls;la++,ls--) {
  ...
  if (j==max_prelook_arcs) {
    if (verbose&show_details)
      fprintf(stderr,"prelook arcs cut off at %d; see option z\n",
           max_prelook_arcs);
...
if (verbose&show_looks) {
  fprintf(stderr,"Looks at level %d:\n",
                            level);
  for (i=0;i<looks;i++)
...
if (level && (o,lmem[looklit].dl_fail!=istamp)) {
  if (lmem[looklit].wnb>dl_trigger) {
    if ((ullng)cs+2*looks*(dl_max_iter+1)<proto_truth) {
...
They now are literals that were forced to be false as a consequence
of |looklit|.
...
if (freevars) {
  ...
  for (k=0;k<freevars;k++) {
    if (verbose&show_unused_vars) printf(" %.8s",vmem[freevar[k]].name.ch8);
    if (out_file) fprintf(out_file," %.8s",vmem[freevar[k]].name.ch8);
  }
