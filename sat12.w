...
main (int argc, char *argv[]) {
  register uint aa,c,cc,h,i,j,k,l,ll,p,pp,q,qq,r,s,t,u,uu,v,vv,w,ww,x;
...
ullng mem_max; /* maximum number of cells allowed in |mem| */
...
void show_cell(uint k) {
  printf("mem["O"u]=",k);
  if (is_lit(k))
    printf("occ "O"u, time "O"u, sig "O"llx, up "O"u, dn "O"u\n",
        occurs(k),littime(k),litsig(k),up(k),down(k));
  else if (is_cls(k))
    printf("size "O"u, time "O"u, sig "O"llx, left "O"u, right "O"u\n",
        size(k),clstime(k),clssig(k),left(k),right(k));
  else printf("lit "O"u, cls "O"u, lft "O"u, rt "O"u, up "O"u, dn "O"u\n",
...
We add a 16-bit |spare| field, so that a |variable| struct fills two octabytes.
...
@<Type...@>=
typedef enum {@!norm,@!elim_quiet,@!elim_res,
  @!forced_true,@!forced_false} status_t;
...
@ Another subroutine, used only for debugging, shows all the clauses
currently in memory.
...
void print_clauses_for(int l) {
  register uint k;
  if (is_lit(l) && l>=2) {
    if (vmem[thevar(l)].status==elim_res) {
...
void sanity(void) {
  ...
  for (l=2,countl=0;is_lit(l);l++)
    if (vmem[thevar(l)].status==0) @<Verify the cells for literal |l|@>;
...
@ I'm using a single hash bit here. However, two bits might conceivably
work better; I plan to experiment, via a change file.
...
for (o,pp=down(l);!is_lit(pp);o,pp=down(pp)) {
  ...
  if (l<=ll) @<Remove the subsumed clause |cc|@>;
}
...
while (1) {
  if (u==l) @<Move |v| left@>;
  if (uu==ll) @<Move |vv| left@>;
  if (u==uu) @<Copy |u| and move both |v| and |vv| left@>@;
  else if (u==bar(uu)) @<Return a tautology@>@;
...
{
  o,vv=left(vv);
  if (is_cls(vv)) @<Copy the rest of |c| and |break|@>;
  o,uu=mem[vv].lit;
}

@ @<Copy the rest of |c| and |break|@>=
{
  while (1) {
    q=p,p=get_cell();
...
\alpha^{(1)}=0$. Then the computation proceeds without any improvement.
...
no literals have a |rank| that matches |stamp|.
...
l=pos_lit(x);
if ((o,occurs(l)>cutoff) && (o,occurs(l+1)>cutoff)) goto elim_done;
clauses_saved=occurs(l)+occurs(l+1);
@<Partition the $\alpha$'s...@>;
if (beta0==0) {
  l++; /* if at first you don't succeed, ... */
  @<Partition the $\alpha$'s...@>;
}
...
``strengthened,'' because we want to exploit their ability to
...
if (vars_gone==vars) {
  ...
  fprintf(stderr,""O"d variable"O"s and "O"d clause"O"s removed.\n",
