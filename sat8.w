...
ullng maxsteps; /* maximum steps per walk ($2500n$ by default) */
int maxtrials=1000000; /* maximum walks to try */
double nongreedprob=0.5; /* the probability bias for nongreedy choices */
...
if (k || hbits<0 || hbits>30 || buf_size<=0) {
  fprintf(stderr,
     "Usage: %s [v<n>] [h<n>] [b<n>] [s<n>] [d<n>] [t<n>] [w<n>] [p<f>] < foo.dat\n",
         argv[0]);
  exit(-1);
}
...
@<Solve the problem@>=
if (maxsteps==0) maxsteps=2500*vars;
...
@ @<Print the initial guess@>=
{
  ...
  for (k=0;k<vars;k++)
    fprintf(stderr," %s%s",vmem[k].val?"":"~",nmem[k].ch8);
  fprintf(stderr,"\n");
}
...
{
  for (k=0;k<vars;k++)
    printf(" %s%s",vmem[k].val?"":"~",nmem[k].ch8);
  printf("\n");
  goto done;
}
