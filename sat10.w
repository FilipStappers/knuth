...
case 'e': k|=(sscanf(argv[j]+1,"%f",&eps)-1);@+break;
...
if (k || hbits<0 || hbits>30 || buf_size<=0) {
  fprintf(stderr,
   "Usage: %s [v<n>] [h<n>] [b<n>] [s<n>] [d<n>] [e<f>] [x<foo>] < foo.dat\n",
                          argv[0]);
  exit(-1);
}
  ...
  for (v=1;v<=vars;v++) {
    for (c=vmem[v].wlist0;c;c=cmem[c].wlink) {
      ...
      if (mem[cmem[c].start]!=v+v)
        fprintf(stderr,"Clause %d watches %u, not %u!\n",
                c,mem[cmem[c].start],v+v);
      else if (val[v]==false)
        fprintf(stderr,"Clause %d watches the false literal %u!\n",
                c,(v+v));
    }
    ...
    if (vmem[v].spare==0 && val[v]==unknown &&
      ...
      fprintf(stderr,"Variable %.8s should be in the active ring!\n",
                vmem[v].name.ch8);
    if (vmem[v].spare==1 && (val[v]!=unknown ||
               ((vmem[v].wlist0|vmem[v].wlist1)==0)))
      fprintf(stderr,"Variable %.8s should not be in the active ring!\n",
                vmem[v].name.ch8);
    vmem[v].spare=0;
  }
  if (k!=clauses)
    fprintf(stderr,"Oops: %d of %lld clauses are being watched!\n",
                           k, clauses);
}
...
void print_state(int l) {
  ...
  for (k=1;k<=l;k++) fprintf(stderr,"%c",smem[k].move+'0');
  fprintf(stderr,"\n");
  fflush(stderr);
}
...
if (j!=cells) {
  fprintf(stderr,"Oh oh, something happened to %d cells!\n",
                   (int)cells-j);
  exit(-15);
}
o,cmem[c].start=j;
...
@ When literal |l| watches clause |c|, we know that |l| is the first
...
for (o,c=vmem[v].wlist0;c;o,c=cmem[c].wlink) {
  ...
  if (verbose&show_details)
    fprintf(stderr,"(Clause %d reduced to %.8s)\n",
                        c,vmem[v].name.ch8);
  force=1;
  goto forced0;
unforced0: continue;
...
if (verbose&show_choices) {
  fprintf(stderr,"Level %d, ",
     level);
  switch (nextmove) {
case 0: fprintf(stderr,"trying %.8s",
                 vmem[v].name.ch8);@+break;
case 1: fprintf(stderr,"trying ~%.8s",
                 vmem[v].name.ch8);@+break;
case 2: fprintf(stderr,"retrying %.8s",
                 vmem[v].name.ch8);@+break;
case 3: fprintf(stderr,"retrying ~%.8s",
                 vmem[v].name.ch8);@+break;
case 4: fprintf(stderr,"forcing %.8s",
                 vmem[v].name.ch8);@+break;
case 5: fprintf(stderr,"forcing ~%.8s",
                 vmem[v].name.ch8);@+break;
  }
...
for (; c;c=cc) {
  ...
  if (k==cmem[c-1].start) {
    fprintf(stderr,"Clause %d can't be watched!\n",
                 c); /* ``can't happen'' */
    exit(-18);
  }
  oo,mem[k]=ll,mem[j]=l;
...
point, because it might not be watching anything else. In such a
...
@<Put |c| into the watch list of |l|@>=
if (verbose&show_details)
  fprintf(stderr,"(Clause %d now watched by %s%.8s)\n",
...
if (level<vars) {
  ...
  for (v=0;v<vars;v++) if (val[v]==unknown) {
    if (verbose&show_unused_vars) printf(" %.8s",vmem[v].name.ch8);
    if (out_file) fprintf(out_file," %.8s",vmem[v].name.ch8);
  }
